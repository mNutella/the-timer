...
export const timeEntriesTotalDurationByClientAndDateAggregate =
	new TableAggregate<{
		Namespace: Id<"users">;
		Key: [string, number];
		DataModel: DataModel;
...
export const timeEntriesTotalDurationByProjectAndDateAggregate =
	new TableAggregate<{
		Namespace: Id<"users">;
		Key: [string, number];
		DataModel: DataModel;
...
export const timeEntriesTotalDurationByCategoryAndDateAggregate =
	new TableAggregate<{
		Namespace: Id<"users">;
		Key: [string, number];
		DataModel: DataModel;
		...
		sortKey: (doc) => [
			doc.categoryId ?? "",
			getStartOfDay(doc.start_time ?? 0),
		],
		sumValue: (doc) => doc.duration ?? 0,
	});
